...
@interface AssetsGroupViewController : UITableViewController
...
@interface AssetsViewCell : UICollectionViewCell
...
@interface AssetsSupplementaryView : UICollectionReusableView
...
@interface AssetsSupplementaryView ()
...
#pragma mark - AssetsPickerController


@implementation AssetsPickerController

- (id)init {
    AssetsGroupViewController *groupViewController = [[AssetsGroupViewController alloc] init];
...
#pragma mark - AssetsGroupViewController
...
- (void)setupButtons {
    AssetsPickerController *picker = (AssetsPickerController *)self.navigationController;
    
    if (picker.showsCancelButton){
        self.navigationItem.rightBarButtonItem =
        [[UIBarButtonItem alloc] initWithTitle:lang(@"Cancel")
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    AssetsGroupViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil)
    {
        cell = [[AssetsGroupViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier];
    }
...
#pragma mark - AssetsGroupViewCell
...
- (id)init{
    ...
    if (self = [super initWithCollectionViewLayout:layout]){
        ...
        [self.collectionView registerClass:[AssetsViewCell class]
                forCellWithReuseIdentifier:kAssetsViewCellIdentifier];
        
        [self.collectionView registerClass:[AssetsSupplementaryView class]
...
- (void)setupButtons{
    self.navigationItem.rightBarButtonItem =
    [[UIBarButtonItem alloc] initWithTitle:lang(@"Done")
                                     style:UIBarButtonItemStylePlain
                                    target:self
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    AssetsViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:CellIdentifier forIndexPath:indexPath];
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    AssetsPickerController *vc = (AssetsPickerController *)self.navigationController;
...
//    }
    ...
    AssetsPickerController *vc = (AssetsPickerController *)self.navigationController;
    self.title = [NSString stringWithFormat:@"%lu/%ld", (unsigned long)indexPaths.count, (long)vc.maximumNumberOfSelection];
}
...
- (void)bind:(ALAsset *)asset{
    ...
    self.title  = [TimeUtil timeDescriptionOfTimeInterval:[[asset valueForProperty:ALAssetPropertyDuration] doubleValue]];
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([self.type isEqual:ALAssetTypeVideo]){
        ...
        NSDictionary *attrs = @{NSFontAttributeName:titleFont, NSForegroundColorAttributeName:titleColor};
        [attributedString addAttributes:attrs range:NSMakeRange(0, self.title.length)];
        [self.title drawAtPoint:CGPointMake(rect.size.width - attributedString.size.width - 2 , startPoint.y + (titleHeight - 12) / 2) withAttributes:attrs];
...
#pragma mark - AssetsSupplementaryView

@implementation AssetsSupplementaryView
