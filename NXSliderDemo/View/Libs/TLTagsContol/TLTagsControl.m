...
@implementation TLTagsControl {
    ...
    NSUInteger                  maxNum; //限制Tags的最大数目，0表示无限制
}
...
//            LogInfo(@"tagInputField_.x:%f, tagInputField_.width:%f, self.width:%f", tagInputField_.frame.origin.x, tagInputField_.frame.size.width, self.frame.size.width);
        } else {
            UIView *lastTag = tagSubviews_.lastObject;
...
- (void)reloadTagSubviews {
    ...
    tagInputField_.font = _tagsTextFont;
    [tagInputField_ setTintColor:_cursorColor];
    ...
    for (NSString *tag in _tags) {
        ...
        if (_mode == TLTagsControlModeEdit) {
            UITapGestureRecognizer* singleTapRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(singleTapRecognizer:)];
            //点击的次数
            singleTapRecognizer.numberOfTapsRequired = 1; // 单击
            [tagView addGestureRecognizer:singleTapRecognizer];
            
            UIButton *deleteTagButton = [[UIButton alloc] initWithFrame:tagInputField_.frame];
            CGRect buttonFrame = deleteTagButton.frame;
            [deleteTagButton.titleLabel setFont:[UIFont systemFontOfSize:CGRectGetHeight(tagInputField_.frame) / 2]];
            deleteTagButton.titleLabel.textAlignment = NSTextAlignmentLeft;
            ...
            buttonFrame.origin.x = labelFrame.size.width - 5;
            buttonFrame.size.width -= CGRectGetHeight(tagInputField_.frame) / 2;
    ...
    if (_mode == TLTagsControlModeEdit) {
        ...
        if (maxNum > 0) {
            if (_tags.count < maxNum) {
                [tagInputField_ setEnabled:YES];
                [tagInputField_ setPlaceholder:_tagPlaceholder];
            } else {
                [tagInputField_ setEnabled:NO];
                [tagInputField_ setPlaceholder:@""];
    //                    frame.origin.x -= 100;
            }
        }
...
#pragma mark - 
- (void)singleTapRecognizer:(UITapGestureRecognizer *)rec {
    [rec.view removeFromSuperview];
    
    NSInteger index = [tagSubviews_ indexOfObject:rec.view];
